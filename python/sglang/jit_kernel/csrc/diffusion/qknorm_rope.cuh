...
// Forward declaration for the large-token staged kernel.
template <int64_t kHeadDim, int64_t kRopeDim, bool kIsNeox, bool kUsePDL, typename DType>
struct QKNormRopeStagedKernel;

// Staging the RoPE cache is beneficial for the large-token bf16 production path.
// Smaller shapes stay on the original warp-per-head kernel to avoid extra CTA work.
...
struct QKNormRopeKernel {
  ...
  run(const tvm::ffi::TensorView q,
    ...
    // Use the staged kernel only for the large-token bf16 production template.
    // Other templates, including fp16 and NeoX layouts, keep the existing warp path.
