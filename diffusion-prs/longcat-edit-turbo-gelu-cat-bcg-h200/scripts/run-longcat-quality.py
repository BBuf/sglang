"""Pinned native LongCat Image Edit Turbo request with saved output and perf evidence."""
import argparse
import fcntl
import gzip
...
os.environ.update(json.loads((root/'artifacts/longcat-edit-turbo-cache-env.json').read_text()))
...
out=root/'artifacts/longcat-image-edit-turbo'/args.label
...
preset=bench.MODELS['longcat-image-edit-turbo']
preset['seed']=42
...
bench.MODELS['longcat-image-edit-turbo']=preset
(out/'preset.json').write_text(json.dumps(preset,indent=2))
result=bench._run_benchmark_once_impl('longcat-image-edit-turbo',args.label,out,warmup=True,torch_compile=False,
    quality=args.quality,breakable_cuda_graph=args.bcg,model_cache_dir=root/'model-caches/longcat-image-edit-turbo-cycle',cuda_visible_devices='0')
