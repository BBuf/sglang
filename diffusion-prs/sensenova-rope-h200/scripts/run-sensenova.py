"""Pinned native SenseNova eager/BCG request with saved output and perf evidence."""
import argparse
import fcntl
import gzip
...
os.environ.update(json.loads((root/'artifacts/sensenova-cache-env.json').read_text()))
...
out=root/'artifacts/sensenova-u1'/args.label
...
preset=dict(path='sensenova/SenseNova-U1.5-8B-MoT',
            prompt='A curious raccoon peers through a vibrant field of yellow sunflowers, its eyes wide with interest.',
            seed=42,force_eager=True,
            extra_args=['--num-gpus=1','--enable-torch-compile=false','--performance-mode=speed',
                        '--width=2048','--height=2048','--num-inference-steps=50','--guidance-scale=4',
                        '--warmup-resolutions=2048x2048'])
if args.profile:
    preset['extra_args']+=['--profile','--profile-all-stages']
    os.environ['SGLANG_DIFFUSION_TORCH_PROFILER_DIR']=str(out/'traces')
bench.MODELS['sensenova-u1']=preset
(out/'preset.json').write_text(json.dumps(preset,indent=2))
result=bench._run_benchmark_once_impl('sensenova-u1',args.label,out,warmup=True,torch_compile=False,
    quality='lossless',breakable_cuda_graph=args.bcg,model_cache_dir=root/'model-caches/sensenova-u1-cycle',cuda_visible_devices='0')
