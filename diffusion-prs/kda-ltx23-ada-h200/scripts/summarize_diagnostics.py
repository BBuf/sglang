...
for path in (a.directory/'traces').glob('*.trace.json*'):
 ...
 kernels=[dict(name=name,count=kernel_counts[name],cumulative_gpu_us=kernel_us[name]) for name in kernel_counts if any(text in name.lower() for text in ['rmsnorm','tanh_residual','cat_pad','nearest_upsample','upsample_nhwc','ada_values','_gn_','group_norm','bias_silu','bias_glu','silu_mul','complex_rope','rotate_half','joint_qkv','hunyuan','qkvs','indexed_'])]
 scopes=Counter()
