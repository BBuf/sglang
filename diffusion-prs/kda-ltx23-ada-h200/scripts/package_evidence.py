...
for name,path in [('baseline',a.baseline_dir),('candidate',a.candidate_dir)]:
 ...
 if (path/'native-config.json').exists():shutil.copy2(path/'native-config.json',target/'native-config.json')
...
for name in ['run_native.py','run_abba.py','compare_media.py','audit_abba.py','summarize_diagnostics.py','package_evidence.py','presets.json']:
...
if request.get('cli_config'):
 geometry=f"native target `{json.dumps(request['cli_config'].get('target'),sort_keys=True)}`, CLI num_inference_steps `{request['cli_config']['num_inference_steps']}`"
else:
 geometry=f"shape `{request['width']}x{request['height']}`, `{request['frames']}` frames, `{request['steps']}` steps"
body=f'''Native model: `{request['model']}@{request['revision']}`; quality `{meta['baseline']['quality']}`; fixed seed `{request['seed']}`; {geometry}. Prompt: `{request['prompt']}`.
