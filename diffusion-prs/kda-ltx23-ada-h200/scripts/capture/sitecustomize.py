...
if os.environ.get('KDA_CAPTURE_DIR'):
    ...
    targets = {
        ...
        'modulate.indexed_modulation_triton': ['indexed_scale_shift_bf16_', 'indexed_gate_bf16_', 'indexed_gate_bf16'],
    }
    def describe(value):
    ...
    def instrument(fn, qualified):
        ...
        def wrapper(*args, **kwargs):
            indexed_numeric = None
            if '.indexed_modulation_triton.' in qualified:
                import torch
                values = list(args) + list(kwargs.values())
                indexed_numeric = dict(inputs_finite=[bool(torch.isfinite(value).all().item()) for value in values])
                indices = args[3] if len(args) >= 4 else kwargs['indices']
                indexed_numeric['indices_min'] = int(indices.min().item()) if indices.numel() else None
                indexed_numeric['indices_max'] = int(indices.max().item()) if indices.numel() else None
            ...
            if indexed_numeric is not None:
                indexed_numeric['output_finite'] = bool(torch.isfinite(result).all().item())
                row['numeric'] = indexed_numeric
            if shadow is not None:
