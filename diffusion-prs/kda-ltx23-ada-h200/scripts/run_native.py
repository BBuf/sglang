...
def main():
    ...
    p.add_argument('--model', choices=('zimage', 'wan21', 'sana-video', 'ltx2', 'flux2-klein', 'hunyuanvideo', 'ltx23', 'fasth3-vsa'), required=True)
    ...
    if config.get('cli_config'):
        assert a.steps is None and a.frames is None, 'Native config owns its model-specific schedule/target'
    ...
    env.update(PYTHONPATH=str(repo / 'python'), CUDA_VISIBLE_DEVICES=a.gpu,
               HF_HUB_CACHE=config.get('hf_hub_cache','/cluster-storage/models'), HF_HUB_OFFLINE='1', TRANSFORMERS_OFFLINE='1',
               FLASHINFER_DISABLE_VERSION_CHECK='1', SGLANG_DIFFUSION_SYNC_STAGE_PROFILING='1', OMP_NUM_THREADS='8')
    if config.get('native_cache_root'):
        env['SGLANG_DIFFUSION_CACHE_ROOT'] = config['native_cache_root']
    if a.capture:
    ...
    if config.get('cli_config'):
        native_config = dict(config['cli_config'])
        assert native_config['backend'] == 'sglang' and native_config['quality'] == a.quality
        assert native_config['model_path'] in (config['model'],config.get('local_model_path')) and native_config['revision'] == config['revision']
        native_config.update(master_port=32000 + int(a.gpu) * 10, scheduler_port=32100 + int(a.gpu) * 10,
                             port=32200 + int(a.gpu) * 10, output_path=str(out), perf_dump_path=str(out / 'perf.json'))
        (out / 'native-config.json').write_text(json.dumps(native_config, indent=2))
        cmd = ['sglang', 'generate', '--backend=sglang', '--model-path=' + native_config['model_path'],
               '--revision=' + config['revision'], '--config=' + str(out / 'native-config.json')]
    else:
    ...
    metadata = dict(source_sha=a.source_sha, repo=str(repo), sglang_file=sglang.__file__, request=config, quality=a.quality,
                    ...
                    environment={key: env[key] for key in ['PYTHONPATH', 'CUDA_VISIBLE_DEVICES', 'HF_HUB_CACHE', 'HF_HUB_OFFLINE', 'FLASHINFER_DISABLE_VERSION_CHECK', 'SGLANG_DIFFUSION_SYNC_STAGE_PROFILING', 'SGLANG_DIFFUSION_CACHE_ROOT'] if key in env})
