"""Optimized ltx2_ada_values9 for H200 (SM90).

The pinned SGLang kernel launches ``B*S`` CTAs, each holding nine
hidden-size registers tiles (nine 8KB loads + nine 8KB stores per CTA for
D=4096).  When ``B*S`` is small (the common LTX2 decode shape, e.g. 1 or 6
rows) the GPU is almost idle and each CTA's serial nine-slice work dominates.

This candidate flattens the ``(row, slice, column-block)`` work space into a
single 1-D grid so every slice/column tile is an independent CTA:

    grid = rows * 9 * cdiv(hidden, BLOCK_N)

For D=4096, B*S=1 this launches 36 CTAs instead of 1, and each CTA moves one
1024-wide tile (~4KB in / 4KB out) instead of nine 4096-wide tiles.  The
outputs still land in one shared ``[9, B, S, D]`` allocation; slice ``p`` for
row ``r`` is written at ``(p * rows + r) * hidden``, which is exactly the
contiguous layout ``torch.empty((9,B,S,D)).unbind(0)`` produces.

Numerics are bit-identical to the baseline: table and timestep elements are
loaded, cast to bfloat16, added with fp32 arithmetic and rounded back to
bfloat16 on store, in the same order.
"""
...
TARGETS = {"ltx2_ada_values9": ltx2_ada_values9}


def run(op, kwargs):
    return TARGETS[op](**kwargs)
