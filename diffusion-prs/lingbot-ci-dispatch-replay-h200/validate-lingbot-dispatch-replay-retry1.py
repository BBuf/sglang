...
root=Path('/campaign');done=root/'validate-cosmos3-super-replication-retry2.exit'
while not done.exists():time.sleep(5)
assert done.read_text().strip()=='0'
repo=root/'candidate-lingbot-dispatch-replay'
subprocess.run(['git','-C',str(root/'baseline-current'),'fetch',str(root/'lingbot-ci-dispatch-replay-incremental.bundle'),'HEAD'],check=True)
