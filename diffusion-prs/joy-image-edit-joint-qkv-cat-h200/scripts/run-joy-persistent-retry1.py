...
def main():
    ...
    out = root / 'artifacts/joy-image-edit/persistent' / args.label
    ...
    (out / 'protocol.json').write_text(json.dumps(dict(argv=argv, saved_warmups=1, measured_requests=5,
        native_request_warmup_steps=40, source=sha, client_wall_definition='perf_counter around generate plus CLI maybe_dump_performance; hashing and campaign JSON excluded'), indent=2))
    ...
    try:
        for index in range(6):
            ...
            params = dict(sampling, request_id=generate_request_id(), output_path=str(out),
                output_file_name=f'request-{index}.png', perf_dump_path=str(perf))
