...
for name in ['audit-joy-qkv-cat-final-retry1', 'analyze-joy-outplace-ncu-v1', 'analyze-joy-outplace-profile-v1']:
    done = root / f'{name}.exit'
    while not done.exists():
        time.sleep(5)
    assert done.read_text().strip() == '0', name
base_sha = '80da4432d085ed4d6166ef643d9fd2b829dbb0c5'
candidate_sha = '4811f4d52aa2586412f699b3bb84ed184d76250a'
copy_only_sha = '3b00154ddb33ac436906a493c9706ed3855ec918'
...
comparisons = {}
for version, head in [('v2', v2_sha), ('final', copy_only_sha), ('outplace-v1', candidate_sha)]:
...
high = [o for o in outputs if o['label'] in ['outplace-v1-baseline-high', 'outplace-v1-candidate-high']]
...
for arm, label in [('baseline', 'baseline-profile-retry1'), ('candidate', 'outplace-v1-candidate-profile')]:
    ...
    cats = [k for k in evidence['top_kernels'] if 'CatArrayBatchedCopy' in k['name'] and 'OpaqueType<2u>' in k['name']]
    fused = [k for k in evidence['top_kernels'] if '_joint_qkv_cat_kernel' in k['name']]
    unchanged_fp32_cats = [k for k in evidence['top_kernels'] if 'CatArrayBatchedCopy' in k['name'] and 'OpaqueType<4u>' in k['name']]
    assert sum(k['count'] for k in unchanged_fp32_cats) == 2
    assert sum(k['count'] for k in cats) == (120 if arm == 'baseline' else 0)
    assert sum(k['count'] for k in fused) == (0 if arm == 'baseline' else 40)
    assert evidence['kernel_count'] == (1268 if arm == 'baseline' else 1108)
    profiles[arm] = dict(model_calls=evidence['model_calls'], kernel_count=evidence['kernel_count'],
        ...
        cats=cats, fused=fused, unchanged_fp32_cats=unchanged_fp32_cats)

report = dict(qualified=all(g['qualifies'] for g in comparisons['outplace-v1'].values()),
    checkpoint='jdopensource/JoyAI-Image-Edit-Diffusers', checkpoint_revision='4b41fb25d961f37668750178ccbb380da326201c',
    baseline=base_sha, candidate=candidate_sha, ncu_candidate=candidate_sha,
    ncu_candidate_difference='Final combined helper, native tests, profile and focused NCU use the same candidate commit.',
    ...
    microbench=json.loads((art / 'outplace-chain-microbench-v1.json').read_text()),
    ncu=json.loads((root / 'profile/joy-outplace-v1-h200/analysis/key-metrics.json').read_text()),
...
old = json.loads((art / 'final-evidence.json').read_text())
assert old['candidate'] == copy_only_sha
(art / 'copy-only-final-evidence.json').write_text(json.dumps(old, indent=2))
...
for index, (label, title) in enumerate([
    ('outplace-fullwarm-r2-baseline-a1', 'Lossless baseline'),
    ('outplace-fullwarm-r2-candidate-b1', 'Lossless joint copy'),
    ('outplace-v1-baseline-high', 'High baseline'),
    ('outplace-v1-candidate-high', 'High joint copy'),
]):
...
candidate_pixels = np.asarray(Image.open(next((art / 'outplace-fullwarm-r2-candidate-b1').glob('*.png'))).convert('RGB'))
