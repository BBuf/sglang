...
with (root/'gpu.lock').open('a') as lock,torch.inference_mode():
 ...
 for batch,image_tokens,text_tokens,heads in ([(1,8048,1004,32)] if args.ncu else [(1,8048,1004,32),(1,2048,256,32),(2,257,13,8)]):
  inputs=[]
 ...
 if not args.ncu:(root/'artifacts/joy-image-edit/qkv-cat-microbench-v1.json').write_text(json.dumps(records,indent=2))
