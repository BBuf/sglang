...
root = Path('/campaign')
prior = root / 'analyze-joy-request-tail.exit'
while not prior.exists():
    time.sleep(5)
assert prior.read_text().strip() == '0'
...
for arm in ['baseline', 'candidate']:
    ...
    report[arm] = dict(rows=rows, components=components, outside_requests=startup,
...
out = root / 'artifacts/joy-image-edit/request-tail-diagnostic-final.json'
