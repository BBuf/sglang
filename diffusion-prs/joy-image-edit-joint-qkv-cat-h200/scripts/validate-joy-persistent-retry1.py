...
for round_index in [1, 2]:
    for arm, suffix in [('baseline', 'a1'), ('candidate', 'b1'), ('candidate', 'b2'), ('baseline', 'a2')]:
        label = f'persistent-retry1-r{round_index}-{arm}-{suffix}'
        ...
        with log_path.open('x') as log:
            code = subprocess.run(['python', '-u', str(root / 'run-joy-persistent-retry1.py'), '--repo', str(repo), '--label', label], stdout=log, stderr=subprocess.STDOUT).returncode
