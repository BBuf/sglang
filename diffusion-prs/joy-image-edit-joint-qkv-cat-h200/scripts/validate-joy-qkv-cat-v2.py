"""Validate guarded joint copies, collect NCU, then measure fixed full-warm ABBA."""
import fcntl
import json
import os
...
root=Path('/campaign');repo=root/'candidate-joy-qkv-cat-v2';art=root/'artifacts/joy-image-edit'
done=root/'validate-joy-qkv-cat-v1.exit'
while not done.exists():time.sleep(5)
assert done.read_text().strip()=='0'
subprocess.run(['git','-C',str(root/'baseline-super-current'),'fetch',str(root/'joy-qkv-cat-v2.bundle'),'HEAD'],check=True)
subprocess.run(['git','-C',str(root/'baseline-super-current'),'worktree','add','--detach',str(repo),'0cff694f701987ba694954a4cf54257f7c3881e6'],check=True)
...
with (root/'gpu.lock').open('a') as lock:
 ...
 assert command('test-joy-qkv-cat-v2',['python','-m','pytest','-q','test/registered/kernels/ops/diffusion/test_joint_qkv_cat.py'],cwd=repo,env=env)==0
assert command('bench-joy-qkv-cat-v2',['python','-u',str(root/'bench-joy-qkv-cat-v2.py')])==0

profile=root/'profile/joy-qkv-cat-v2-h200';profile.mkdir(exist_ok=False)
for child in ['harness','reports','analysis']:(profile/child).mkdir()
shutil.copy2(root/'bench-joy-qkv-cat-v2.py',profile/'harness/bench-joy-qkv-cat-v2.py')
...
for arm in ['baseline','candidate']:
 args=[ncu,'--set','full','--section','PmSampling','--target-processes','all','--nvtx','--nvtx-include','profile/',
  '-o',str(profile/'reports'/arm),'python',str(profile/'harness/bench-joy-qkv-cat-v2.py'),'--ncu',arm]
 assert command(f'ncu-joy-qkv-cat-v2-{arm}',args)==0
assert command('ncu-joy-qkv-cat-v2-candidate-source',[ncu,'--set','source','--section','SourceCounters',
 '--target-processes','all','--nvtx','--nvtx-include','profile/','-o',str(profile/'reports/candidate-source'),
 'python',str(profile/'harness/bench-joy-qkv-cat-v2.py'),'--ncu','candidate'])==0
(root/'ncu-joy-qkv-cat-v2.exit').write_text('0')
...
for round in [1,2]:
 for arm,suffix in [('baseline','a1'),('candidate','b1'),('candidate','b2'),('baseline','a2')]:
  label=f'qkv-fullwarm-r{round}-{arm}-{suffix}'
  ...
  (art/'qkv-abba-fullwarm-v2.json').write_text(json.dumps(records,indent=2))
(root/'measure-joy-qkv-cat-v2.exit').write_text('0')

for label,which,flags in [
 ('qkv-v2-candidate-profile',repo,['--profile']),
 ('qkv-v2-baseline-high',root/'baseline-super-current',['--quality','high']),
 ('qkv-v2-candidate-high',repo,['--quality','high']),
 ('qkv-v2-candidate-bcg',repo,['--bcg']),
 ('qkv-v2-baseline-high-bcg',root/'baseline-super-current',['--quality','high','--bcg']),
 ('qkv-v2-candidate-high-bcg',repo,['--quality','high','--bcg']),
]:
...
assert json.loads((art/'qkv-v2-baseline-high/result.json').read_text())['output_sha256']==json.loads((art/'qkv-v2-candidate-high/result.json').read_text())['output_sha256']
