"""Pinned native FLUX.2 Klein base 4B saved-output benchmark and diagnostic request."""
import argparse
import fcntl
import gzip
...
os.environ.update(json.loads((root/'artifacts/flux2-klein-base-4b-cache-env.json').read_text()))
os.environ.update(HF_HUB_OFFLINE='1',TRANSFORMERS_OFFLINE='1',PYTHONPATH=str(repo/'python'),
    OMP_NUM_THREADS='8',CUDA_VISIBLE_DEVICES='0',SGLANG_DIFFUSION_SYNC_STAGE_PROFILING='1')
...
selected={line.split(',')[1].strip() for line in inventory.splitlines() if line.split(',')[0].strip() in {'0'}}
for _ in range(30):
...
out=root/'artifacts/flux2-klein-base-4b'/args.label
...
preset=dict(path='black-forest-labs/FLUX.2-klein-base-4B',
    prompt='A futuristic cyberpunk city at night, neon lights reflecting on wet streets',seed=42,
    env={},
    extra_args=['--width=1024','--height=1024','--num-frames=1','--num-inference-steps=50',
        '--guidance-scale=4','--num-gpus=1','--tp-size=1','--ulysses-degree=1',
        ...
        '--warmup-resolutions=1024x1024','--warmup-num-frames=1',f'--warmup-steps={args.warmup_steps}'])
if args.profile:
...
result=bench._run_benchmark_once_impl(key,args.label,out,warmup=True,torch_compile=False,
    ...
    model_cache_dir=root/'model-caches/flux2-klein-base-4b-cycle',cuda_visible_devices='0')
