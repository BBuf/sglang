...
root=Path('/campaign');repo=root/'baseline-super-current'
...
with (root/'gpu.lock').open('a') as lock,torch.inference_mode():
 ...
 for batch,tokens in [(1,4608),(2,257)]:
  ...
  for magnitude in [.005,1.,200.]:
   ...
   for rounded in [False,True]:
    ...
    out=apply_qk_norm_rope(cq.unflatten(-1,(24,128)),ck.unflatten(-1,(24,128)),normq,normk,128,cache,
                         allow_strided_qk=True,round_norm_before_rope=rounded)
 ...
 (root/'artifacts/flux2-klein-base-4b/qk-rounding-probe.json').write_text(json.dumps(records,indent=2))
