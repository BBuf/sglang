...
root=Path('/campaign/profile/klein-qk-rope-v2-h200')
done=root/'candidate-ncu.exit'
...
for arm in ['baseline','candidate']:
 rep=ncu_report.load_report(str(root/f'{arm}.ncu-rep'));rng=rep.range_by_idx(0);rows=[]
 for idx in range(rng.num_actions()):
  ...
  (root/f'{arm}-{idx}-all-metrics.json').write_text(json.dumps(all_metrics,indent=1,default=str))
  selected=keys+[k for k in all_metrics if 'warp_issue_stalled' in k and k.endswith('pct')]
  rows.append(dict(name=action.name(),metrics={k:all_metrics[k] for k in selected if k in all_metrics}))
 assert len(rows)==(6 if arm=='baseline' else 1),(arm,len(rows))
...
(root/'key-metrics.json').write_text(json.dumps(reports,indent=2))
