...
root=Path('/campaign');repo=root/'candidate-klein-qk-signbit'
assert (root/'screen-klein-qk-integration.exit').read_text().strip()=='0'
subprocess.run(['git','-C',str(root/'baseline-super-current'),'fetch',str(root/'klein-qk-signbit.bundle'),'HEAD'],check=True)
subprocess.run(['git','-C',str(root/'baseline-super-current'),'worktree','add','--detach',str(repo),'38e0360aa5a33571b3e0e9e254c93da02493b77e'],check=True)
with (root/'gpu.lock').open('a') as lock:
 ...
 with (root/'test-klein-qk-v2.log').open('x') as log:
 ...
 (root/'test-klein-qk-v2.exit').write_text(str(result.returncode));assert result.returncode==0

ncu='/opt/nvidia/nsight-compute/2025.3.1/ncu';profile=root/'profile/klein-qk-rope-v2-h200';profile.mkdir(exist_ok=False)
for arm in ['baseline','candidate']:
 with (profile/f'{arm}-ncu.log').open('x') as log:
  result=subprocess.run([ncu,'--set','full','--target-processes','all','--nvtx','--nvtx-include','profile/','--force-overwrite','-o',str(profile/arm),'python',str(root/'bench-klein-qk-v2.py'),'--ncu',arm],stdout=log,stderr=subprocess.STDOUT)
...
for round in [1,2]:
 for arm,suffix in [('baseline','a1'),('candidate','b1'),('candidate','b2'),('baseline','a2')]:
  label=f'qk-v2-r{round}-{arm}-{suffix}'
  ...
  (root/'artifacts/flux2-klein-base-4b/qk-abba-v2.json').write_text(json.dumps(records,indent=2))
