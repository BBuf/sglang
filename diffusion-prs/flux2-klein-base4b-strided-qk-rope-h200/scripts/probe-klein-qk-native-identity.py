...
with (root/'gpu.lock').open('a') as lock,torch.inference_mode():
 ...
 for batch,tokens in [(1,4608),(2,257)]:
  ...
  angles=torch.zeros(tokens,64,device='cuda')
 ...
 (root/'artifacts/flux2-klein-base-4b/qk-native-triton-identity-probe.json').write_text(json.dumps(records,indent=2))
