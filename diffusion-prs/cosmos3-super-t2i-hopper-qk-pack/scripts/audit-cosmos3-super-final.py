...
done = root / 'validate-cosmos3-super-final.exit'
...
report=dict(qualified=True,baseline=baseline,candidate=candidate,groups=groups,profiles=profiles,
    ...
    note='Two fixed ABBA groups on pinned sources. All native saved-output requests retained; failed offline admission and disabled BCG rows are not performance evidence. Profile and kernel CUDA Graph measurements do not claim model BCG support.')
