"""Pinned native Cosmos3 Super Text2Image saved-output benchmark and diagnostic request."""
import argparse
import fcntl
import gzip
...
p.add_argument('--repo',default='/campaign/baseline-current')
...
out=root/'artifacts/cosmos3-super-t2i'/args.label
...
key='cosmos3-super-t2i'
preset=dict(path='nvidia/Cosmos3-Super-Text2Image',
    prompt='A warehouse robot folds a blue cloth on a clean workbench.',seed=42,
    env={'SGLANG_DISABLE_COSMOS3_GUARDRAILS':'1'},
    extra_args=['--width=1024','--height=1024','--num-frames=1','--num-inference-steps=35',
        '--guidance-scale=4','--num-gpus=2','--tp-size=2','--ulysses-degree=1',
        '--enable-torch-compile=false','--performance-mode=manual',
        '--dit-cpu-offload=false','--dit-layerwise-offload=false','--vae-cpu-offload=false',
...
result=bench._run_benchmark_once_impl(key,args.label,out,warmup=True,torch_compile=False,
    ...
    model_cache_dir=root/'model-caches/cosmos3-super-t2i-cycle',cuda_visible_devices='0,1')
