"""Pinned native LongLive2 saved-output benchmark and diagnostic request."""
import argparse
import fcntl
import gzip
...
os.environ.update(json.loads((root/'artifacts/longlive2-cache-env.json').read_text()))
...
out=root/'artifacts/longlive2'/args.label
...
key='longlive2-'+args.mode
preset=bench.MODELS[key]
preset['seed']=42
if args.mode=='i2v':
    preset['image_path']=str(root/'artifacts/input-media/longlive2-cat.png')
resolution='832x480' if args.mode=='t2v' else '960x928'
frames=61
preset['extra_args'] += ['--num-gpus=1','--tp-size=1','--enable-torch-compile=false',
    '--warmup-resolutions='+resolution,'--warmup-num-frames='+str(frames)]
if args.profile:
    # The native LongLive causal loop records StageProfiler metrics but does
    # not call SGLDiffusionProfiler.step_denoising_step. Use the supported full
...
result=bench._run_benchmark_once_impl(key,args.label,out,warmup=True,torch_compile=False,
    ...
    model_cache_dir=root/'model-caches/longlive2-cycle',cuda_visible_devices='0')
