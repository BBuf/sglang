...
label='current-realtime-candidate-retry1'
with (root/f'lingbot-{label}.log').open('x') as out:
    result=subprocess.run(['python','-u',str(root/'run-lingbot-realtime.py'),'--repo',str(root/'candidate-lingbot-current'),'--label',label,'--port','23881','--master-port','23981'],stdout=out,stderr=subprocess.STDOUT)
...
b=json.loads((art/'current-realtime-candidate-retry1/result.json').read_text())
