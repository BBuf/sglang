...
all_reports={}
for provider in ('norm1-baseline','norm1-fused','cross_norm-baseline','cross_norm-fused'):
    ...
    assert len(actions)==({'norm1-baseline':6,'cross_norm-baseline':3}.get(provider,1)),(provider,len(actions))
...
for site in ('norm1','cross_norm'):
    rep=ncu_report.load_report(str(run/f'reports/source-{site}-fused.ncu-rep'));a=rep.range_by_idx(0).action_by_idx(0)
    ...
    for name in a.metric_names():
        ...
        for i in range(m.num_instances()):
            ...
            lines[loc][name]+=v
    hotspots=[dict(file=k[0],line=k[1],primary_samples=sum(n for name,n in v.items() if not name.endswith('_not_issued')),not_issued_subset_samples=sum(n for name,n in v.items() if name.endswith('_not_issued')),stalls=dict(v)) for k,v in lines.items()]
    hotspots.sort(key=lambda r:-r['primary_samples'])
    (run/f'analysis/stall-hotspots-{site}-fused.json').write_text(json.dumps(hotspots,indent=2))
