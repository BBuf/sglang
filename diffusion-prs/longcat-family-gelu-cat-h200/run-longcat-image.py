"""Pinned native LongCat Image T2I request with saved output and perf evidence."""
import argparse
import fcntl
import gzip
...
os.environ.update(json.loads((root/'artifacts/longcat-image-cache-env.json').read_text()))
...
out=root/'artifacts/longcat-image'/args.label
...
preset=bench.MODELS['longcat-image']
preset['seed']=42
preset['extra_args'] += [
...
bench.MODELS['longcat-image']=preset
(out/'preset.json').write_text(json.dumps(preset,indent=2))
result=bench._run_benchmark_once_impl('longcat-image',args.label,out,warmup=True,torch_compile=False,
    quality=args.quality,breakable_cuda_graph=args.bcg,model_cache_dir=root/'model-caches/longcat-image-cycle',cuda_visible_devices='0')
