"""Pinned native SANA Video saved-output benchmark and diagnostic request."""
import argparse
import fcntl
import gzip
...
os.environ.update(json.loads((root/'artifacts/sana-video-cache-env.json').read_text()))
...
out=root/'artifacts/sana-video'/args.label
...
preset['seed']=42
preset['extra_args'] += ['--num-frames=81','--num-inference-steps=50']
resolution='832x480'
frames=81
...
result=bench._run_benchmark_once_impl(key,args.label,out,warmup=True,torch_compile=False,
    ...
    model_cache_dir=root/'model-caches/sana-video-cycle',cuda_visible_devices='0')
