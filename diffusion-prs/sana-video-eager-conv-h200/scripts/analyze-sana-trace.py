"""Summarize a SANA Video profile and a complete two-pass CFG iteration."""
import collections
import gzip
import json
...
models=sorted((e for e in events if e.get('name')=='nn.Module: SanaVideoTransformer3DModel_0'),key=lambda e:e['ts'])
...
if models:
    ...
    assert len(graphs) == 8, len(graphs)
    lo, hi = graphs[2]['ts'], graphs[4]['ts']
    boundary = 'Third to fifth graph start: two single-segment model calls per CFG iteration'
cpu_lo, cpu_hi = lo, hi
...
boundary = 'Complete second CFG iteration; CPU third-to-fifth model or single-segment graph starts joined to GPU kernels by CUDA launch correlation. GPU first-kernel to next-iteration first-kernel boundaries.' 
selected=[]
for e in events:
    ts=e.get('ts')
    if e.get('ph')=='M':
...
report=dict(source=str(source),source_events=len(events),slice=str(target),slice_events=len(selected),
            model_calls=len(models) or len(graphs),boundary=boundary,
