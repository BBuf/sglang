"""Download pinned Cosmos3 Edge only after the LongCat T2I cache is removed."""
...
root=Path('/campaign')
assert not (root/'model-caches/longcat-image-cycle').exists(), 'Finish LongCat Image and audit cleanup first'
...
cache=cache_root/'cosmos3-edge-cycle'
if not cache.exists():
    cache=bench._prepare_model_cache(cache_root,'cosmos3-edge','cycle')
else:
...
(root/'artifacts/cosmos3-edge-cache-env.json').write_text(json.dumps(env,indent=2))
...
(root/'artifacts/cosmos3-edge-snapshot.json').write_text(json.dumps(dict(repo=repo,revision=revision,snapshot=path),indent=2))
print('Pinned Cosmos3 Edge checkpoint ready:',path,flush=True)
