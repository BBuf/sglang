"""Pinned native Cosmos3 Edge saved-output benchmark and diagnostic request."""
import argparse
import fcntl
import gzip
...
p.add_argument('--mode',choices=('t2i','t2v'),required=True)
...
os.environ.update(json.loads((root/'artifacts/cosmos3-edge-cache-env.json').read_text()))
...
out=root/'artifacts/cosmos3-edge'/args.label
...
key='cosmos3-edge-'+args.mode
preset=bench.MODELS[key]
resolution='640x640' if args.mode=='t2i' else '832x480'
frames=1 if args.mode=='t2i' else 81
...
result=bench._run_benchmark_once_impl(key,args.label,out,warmup=True,torch_compile=False,
    ...
    model_cache_dir=root/'model-caches/cosmos3-edge-cycle',cuda_visible_devices='0')
